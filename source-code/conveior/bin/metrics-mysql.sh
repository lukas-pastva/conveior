...
do
  ...
  if [[ "${POD}" != "" ]]; then
    ...
    do
      QUERY=$(yq e ".conveior-config.metrics.pods_mysql | with_entries(select(.value.name == \"$POD_SHORT\")) | .[].queries | with_entries(select(.value.name == \"$QUERY_NAME\" )) | .[].query" /home/conveior-config.yaml)
      if [[ "${QUERY^^}" != *"DROP"* ]]; then
