...
PGPASSWORD=$(cat $POSTGRES_PASSWORD_FILE) psql -U gitlab -d postgres -c "SELECT datname FROM pg_database;" | awk 'NR>3 {print last} {last=$0}'

    export DATABASE_ITEMS=$(PGPASSWORD=$(cat $POSTGRES_PASSWORD_FILE) psql -U gitlab -d postgres -c "SELECT datname FROM pg_database;" | awk 'NR>3 {print last} {last=$0}' | awk -F " " '{print $1}' | head -n -1)
    export IFS=$'\n'
    ...
    if [[ "${DATABASES_STR}" != "" ]]; then
      docker exec -i "${POD}" bash -c "PGPASSWORD=$(cat $POSTGRES_PASSWORD_FILE) pg_dumpall -U ${SQL_USER} > /tmp/${FILE}"
