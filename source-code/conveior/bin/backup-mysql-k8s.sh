...
export POD_SHORT_LIST=$(yq e '.config.backups.dbs_mysql.[].name' ${CONFIG_FILE_DIR})
export IFS=$'\n'
for POD_SHORT in $POD_SHORT_LIST;
do
  echo_message "Backing up ${POD_SHORT}"

  export POD_NAMESPACE=$(yq e ".config.backups.dbs_mysql | with_entries(select(.value.name == \"$POD_SHORT\")) | .[].namespace" ${CONFIG_FILE_DIR})
