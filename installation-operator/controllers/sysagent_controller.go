...
import (
	...
	mydomainv1alpha1 "tronic.sk/ConveiorOperator/api/v1alpha1"
)

// ConveiorReconciler reconciles a Conveior object
type ConveiorReconciler struct {
	client.Client
...
// the Conveior object against the actual cluster state, and then
// perform operations to make the cluster state reflect the state specified by
// the user.
//
// For more details, check Reconcile and its Result here:
// - https://pkg.go.dev/sigs.k8s.io/controller-runtime@v0.9.2/pkg/reconcile
func (r *ConveiorReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := log.FromContext(ctx).WithValues("Conveior", req.NamespacedName)

	// Fetch the Conveior instance
	instance := &mydomainv1alpha1.Conveior{}
...
func (r *ConveiorReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&mydomainv1alpha1.Conveior{}).
		Complete(r)
}
