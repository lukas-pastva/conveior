...
do
  ...
  if [[ "${POD}" != "" ]]; then
    ...
    export PATH=$(yq e ".backups.files | with_entries(select(.value.name == \"$POD_SHORT\")) | .0.path" /home/conveior-config.yaml)
    ...
    if [[ $(docker exec -i "${POD}" bash -c "cd ${PATH} && ls | wc -l") != "0" ]]; then
      docker cp ${POD}:${PATH} ${SERVER_DIR}
      export LAST_DIR=$(echo ${PATH} | awk -F"/" '{print $NF}')
      cd "${SERVER_DIR}/${LAST_DIR}" && zip -rqq "${ZIP_FILE}" "."

      upload_file "${ZIP_FILE}" "backup-file/${POD_SHORT}/${ZIP_FILE_ONLY}"
      find "${SERVER_DIR}" -mindepth 1 -delete
    else
      echo_prom_helper "Empty directory ${PATH}, nothing to backup"
    fi
  fi
